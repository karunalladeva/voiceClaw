...
export const cryptoTickerTool = tool(
  async ({ symbol, exchangeId = 'binance' }) => {
...
export const cryptoOhlcvTool = tool(
  async ({ symbol, exchangeId = 'binance', timeframe = '1h', limit = 200 }) => {
    const config = configManager.getConfig();
    if (!config.marketData.enableCcxt) {
      return JSON.stringify({
        symbol,
