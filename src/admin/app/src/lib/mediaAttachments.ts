...
export function isMarkdownImageUrl(text: string, url: string): boolean {
  const normalized = normalizeMediaUrl(url)
  return text.includes(`![`) && text.includes(`](${normalized})`)
}

export function attachmentsNotInMarkdown(text: string, attachments: MediaAttachment[]): MediaAttachment[] {
  return attachments.filter((attachment) => !isMarkdownImageUrl(text, attachment.url))
}
